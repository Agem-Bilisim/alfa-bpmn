 ...
 *
 */
public class AlfaBpmnProcessEngine 
{

	private final static Logger logger = Logger.getLogger(AlfaBpmnProcessEngine.class);

	private static AlfaBpmnProcessEngine instance;

	private static ProcessEngineConfiguration pec;
	private static ProcessEngine processEngine;

	private AlfaBpmnProcessEngine() {
	}
	 ...
	 * @return
	 */
	public static AlfaBpmnProcessEngine getInstance() 
	{
		if (instance == null) {
			instance= new AlfaBpmnProcessEngine();
			ProcessEngines.init();
			...
			logger.debug("AlfaBpmnProcessEngine instance is created...");
	...
	{
		return AlfaBpmnProcessEngine.processEngine;
	}
	...
	public ProcessDefinition deployModel(String name, String resourcePath)
	{
		...
		if (deployment != null) {
			...
			throw new AlfaBpmnProcessEngineException("Unable to deploy resource with path " + resourcePath);
		}
	}
	...
	{
		if (claim) {
			AlfaBpmnProcessEngine.processEngine.getTaskService()
			.claim(taskId, Utils.getInstance().isEmpty(claimUsername) ? 
					Utils.getInstance().getCurrentUser() : claimUsername);
		}

		if (variables != null) {
			AlfaBpmnProcessEngine.processEngine.getTaskService().complete(taskId, variables, localVariables);
		} else {
			AlfaBpmnProcessEngine.processEngine.getTaskService().complete(taskId);
		}
	}
	...
	{
		AlfaBpmnProcessEngine.processEngine.getTaskService().claim(taskId, claimUsername);
	}
	...
	private List<HistoricProcessInstance> queryProcesses(String processKey, boolean unfinished, boolean finished) 
	{
		...
		if (!Utils.getInstance().isEmpty(processKey)) {
	...
	private List<HistoricProcessInstance> queryProcesses(
		...
		if(!Utils.getInstance().isEmpty(username)){
			query.involvedUser(username);
		}

		if(!Utils.getInstance().isEmpty(beginStartDate)){
			query.startedAfter(beginStartDate);
		}

		if(!Utils.getInstance().isEmpty(endStartDate)){
	...
	public void deleteProcessDeploymentWithProcessKey(String processKey) {
		if (Utils.getInstance().isEmpty(processKey)) {
			throw new AlfaBpmnProcessEngineException("Process key cannot be empty!");
		}
		try {
			...
			throw new AlfaBpmnProcessEngineException(e);
		}
	}
	...
	public void deleteProcessDeploymentsById(String deploymentId) {
		if (Utils.getInstance().isEmpty(deploymentId)) {
			throw new AlfaBpmnProcessEngineException("Process deployment id cannot be empty!");
		}
		try {
	...
	{
		if (Utils.getInstance().isEmpty(taskId)) {
			throw new AlfaBpmnProcessEngineException("Task ID key cannot be empty!");
		}
		try {
	...
	public Task getTaskWithVariables(String processDefinitionKey, String taskDefinitionKey, Map<String, Object> variables, String candidateUser, String assignee) 
	{
		try{
			...
			throw new AlfaBpmnProcessEngineException("Unexpected error", e);
		}
	}
	...
	public HistoricProcessInstance getHistoricProcessInstance(String processId)
	{
		...
		if(processes.size() <= 0) {
			...
			throw new AlfaBpmnProcessEngineException("No Historic Process Exists with id : " + processId);
		}
	...
	public void getProcessDiagram(String processDefinitionKey, OutputStream out)
	{
		...
		try {
			...
			throw new AlfaBpmnProcessEngineException("Error in getting process diagram with key : " + processDefinitionKey, e);
		}		
	}
	...
	public void getProcessDiagramById(String processDefinitionId, OutputStream out)
	{
		...
		try {
			...
			throw new AlfaBpmnProcessEngineException("Error in getting process diagram with id : " + processDefinitionId, e);
		}		
	}
	...
	public void getProcessDiagramForInstance(String processId, OutputStream out, Color ownedColor, List<String> ownedTaskIdList, boolean onlyOwnedTasks)
	{
		...
		DiagramLayout layout = processEngine.getRepositoryService().getProcessDiagramLayout(process.getProcessDefinitionId());
		...
		try {
			...
			for (Task task : tasks) {
				...
				if (highlightTask) {
					DiagramNode node = layout.getNode(task.getTaskDefinitionKey());
					if (node != null) {
			...
			throw new AlfaBpmnProcessEngineException("Error in getting process diagram for process id : " + processId, e);
		}	
	}
		...
		try {
			...
			for (Task task : tasks) {
				...
				if (highlightTask) {
					DiagramNode node = layout.getNode(task.getTaskDefinitionKey());
					if (node != null) {
						imagemap.put(task.getId(), node.getX().intValue() + "," + node.getY().intValue() + "," + (node.getX().intValue() + node.getWidth().intValue()) + "," + (node.getY().intValue() + node.getHeight().intValue()));
						graphics.drawRoundRect(node.getX().intValue(), node.getY().intValue(), node.getWidth().intValue(), node.getHeight().intValue(), 5, 5);
					}
				}
			}
		...
		try {
			...
			for (Task task : tasks) {
				...
				if (highlightTask) {
					DiagramNode node = layout.getNode(task.getTaskDefinitionKey());
					if (node != null) {
						imagemap.put(task, node.getX().intValue() + "," + node.getY().intValue() + "," + (node.getX().intValue() + node.getWidth().intValue()) + "," + (node.getY().intValue() + node.getHeight().intValue()));
						graphics.drawRoundRect(node.getX().intValue(), node.getY().intValue(), node.getWidth().intValue(), node.getHeight().intValue(), 5, 5);
					}
	...
	{	
		if (processInstanceIds.isEmpty()) {
			throw new AlfaBpmnProcessEngineException("Process Instance Ids is empty");
		}
